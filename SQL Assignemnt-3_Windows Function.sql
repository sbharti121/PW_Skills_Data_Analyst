...
SELECT 
    f.film_id, SUM(p.amount) AS cumulative_revenue   -- need to join film,rental,inventory & payment table to get film_id & amount--
FROM
    film AS f
        inner JOIN
...
select title , avg(rental_duration) over(partition by length) avarage_rental_duration from film;



-- Question 4. **Identify the top 3 films in each category based on their rental counts.**

select title, rank() over(order by count(r.rental_id) desc) Category_wise_rank from film as f  -- join film, film_category & rental table to get result
...
film_count as (select title, category_id ,rental_count, avg(rental_count) over (partition by category_id) as avg_count 
from Category_count) 
select  title, category_id ,rental_count, avg_count from film_count where rental_count < avg_count; 

-- Note-  I have print all the column like  title, category_id ,rental_count, avg_count to understand 
-- as per question we need to print only title--
